...
if (!program.githubRepo) {
  ...
  process.exit(1)
} else if (!program.branch) {
  console.error(`Missing ${chalk.red.bold('branch')} parameter, use ${chalk.yellow('`-b`')} or ${chalk.yellow('`--branch`')}`)
  process.exit(1)
...
scraper.createStaticFiles(ghostUrl, staticPath)
  ...
  .then(() => ghpagesdeploy.publish(staticPath, program.githubRepo, program.branch))
